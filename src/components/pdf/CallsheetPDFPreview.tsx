...
export const CallsheetPDFPreview: React.FC<CallsheetPDFPreviewProps> = ({
  ...
  // Calculate proper content dimensions with proper margins
  ...
  const contentStyle: React.CSSProperties = {
    padding: `${customization.layout.margins.top}px ${customization.layout.margins.right}px ${customization.layout.margins.bottom + 40}px ${customization.layout.margins.left}px`,
    minHeight: `calc(${isLandscape ? '210mm' : '297mm'} - ${customization.layout.margins.top + customization.layout.margins.bottom + 40}px)`,
    boxSizing: 'border-box',
    position: 'relative'
  };
  ...
  const getLogoPosition = (position: string) => {
    ...
    switch (position) {
      case 'top-left':
        return { top: `${customization.layout.margins.top + baseOffset}px`, left: `${customization.layout.margins.left + baseOffset}px` };
      case 'top-center':
        return { top: `${customization.layout.margins.top + baseOffset}px`, left: '50%', transform: 'translateX(-50%)' };
      case 'top-right':
        return { top: `${customization.layout.margins.top + baseOffset}px`, right: `${customization.layout.margins.right + baseOffset}px` };
      case 'center-left':
        return { top: '50%', left: `${customization.layout.margins.left + baseOffset}px`, transform: 'translateY(-50%)' };
      case 'center-right':
        return { top: '50%', right: `${customization.layout.margins.right + baseOffset}px`, transform: 'translateY(-50%)' };
      case 'bottom-left':
        return { bottom: `${customization.layout.margins.bottom + baseOffset}px`, left: `${customization.layout.margins.left + baseOffset}px` };
      case 'bottom-center':
        return { bottom: `${customization.layout.margins.bottom + baseOffset}px`, left: '50%', transform: 'translateX(-50%)' };
      case 'bottom-right':
        return { bottom: `${customization.layout.margins.bottom + baseOffset}px`, right: `${customization.layout.margins.right + baseOffset}px` };
      default:
        return { top: `${customization.layout.margins.top + baseOffset}px`, left: `${customization.layout.margins.left + baseOffset}px` };
    }
  };
    ...
    >
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          ...
          /* Ensure proper content boundaries */
          .content-area {
            width: 100%;
            height: auto;
      ...
      {/* Footer - positioned at the bottom of the page within margins */}
      {customization.branding?.footer?.text && (
        <div style={{
          position: 'absolute',
          bottom: `${customization.layout.margins.bottom}px`,
          left: customization.branding.footer.position === 'left' ? `${customization.layout.margins.left}px` : 
                customization.branding.footer.position === 'right' ? 'auto' : '50%',
          right: customization.branding.footer.position === 'right' ? `${customization.layout.margins.right}px` : 'auto',
          ...
          width: customization.branding.footer.position === 'center' ? 'auto' : 'auto',
          maxWidth: customization.branding.footer.position === 'center' ? 
            `calc(${isLandscape ? '297mm' : '210mm'} - ${customization.layout.margins.left + customization.layout.margins.right}px)` : 'auto'
        }}>
          {customization.branding.footer.text}
